...
function Auth(this: any, {email, phone, id}:{email:string, phone:string, id:string}):object {
  try {
    ...
    return Object.defineProperties(this, {
      ...
      get: {
        value: async function getFactor(sid:string) {
          try {
      ...
      verify: {
        value: async function verifyFactor(code:string) {
          try {
      ...
      authenticate: {
        value: async function challengeFactor(code:string, details = null) {
          try {
            if (!this.sid) throw new Error('factor sid not yet set.');
            const options = {
              authPayload: code,
...
// HELPER FUNCTIONS
async function sendQRCode(email: string, secrets: {key: string, uri: string}) {
  try {
    const { uri, key }: {uri: string, key: string} = secrets;
