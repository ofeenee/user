...
const { TWILIO_API_KEY: twilioApiKey, TWILIO_AUTH_TOKEN: twilioAuthToken, TWILIO_SERVICE_SID: twilioServiceSid } = process.env;
const client = (0, twilio_1.default)(twilioApiKey, twilioAuthToken);
const service = client.verify.services(twilioServiceSid);
const mail_1 = __importDefault(require("@sendgrid/mail"));
const { SENDGRID_API_KEY: sendgridApiKey } = process.env;
const mail = mail_1.default.setApiKey(sendgridApiKey);
const qrcode_svg_1 = __importDefault(require("qrcode-svg"));
const validator_1 = __importDefault(require("validator"));
const { isUUID, isEmail, isMobilePhone } = validator_1.default;
function Auth({ email, phone, id }) {
    try {
        if (!new.target)
            return new Auth(id);
        if (id === null || typeof id !== 'string' || isUUID(id) === false)
            throw new Error('Invalid ID');
        if (email === null || typeof email !== 'string' || isEmail(email) === false)
            throw new Error('Invalid Email');
        if (phone === null || typeof phone !== 'string' || isMobilePhone(phone, 'any', { strictMode: true }) === false)
        ...
        return Object.defineProperties(this, {
                ...
                value: async () => {
                    try {
                        ...
                        if (factor.sid) {
                            ...
                            throw new Error('failed to create new authentication factor.');
                    }
                    catch (error) {
                        throw error;
                    }
            ...
            get: {
                value: async function getFactor(sid) {
                    try {
                        if (!sid)
                            throw new Error('Invalid factor sid');
            ...
            verify: {
                value: async function verifyFactor(code) {
                    try {
                        if (!this.sid)
                            throw new Error('factor sid not yet set.');
                        if (!code)
                            throw new Error('Invalid code');
                        const response = await service.entities(id).factors(this.sid).update({ authPayload: code });
                        return response;
                    }
                    catch (error) {
                        throw error;
            ...
            delete: {
                value: async function deleteFactor() {
                    try {
                        if (!this.sid)
                            throw new Error('factor sid not yet set.');
                        const response = await service.entities(id).factors(this.sid).remove();
                        return response;
                    }
                    catch (error) {
                        throw error;
            ...
            listAttempts: {
                value: async function listChallenges(limit = 50) {
                    try {
                        if (!this.sid)
                            throw new Error('factor sid not yet set.');
                        const response = await service.entities(id).challenges.list({ limit: limit });
                        return response;
                    }
                    catch (error) {
                        throw error;
